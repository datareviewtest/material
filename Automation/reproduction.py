...
def reproduce_bug(device_port, reprot_file_name): 
    ...
    history = load_training_prompts('guidelines.json')
    history.append({"role": "user", "content": f"Bug Report: {bug_report}"})
    ...
    log_and_save_history(reprot_file_name, start_time, response_time, total_commands, history, package_name, 'crash')
    device.set_orientation("natural")
